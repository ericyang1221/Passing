...
public class TextToSql_kana {
	...
	public void doWord() throws Exception {
		while (br.ready()) {
			if (readbuffer.charAt(0) != '\t') {
				...
				if (currentKanaStr.charAt(currentKanaStr.length() - 1) > '0'
						&& currentKanaStr.charAt(currentKanaStr.length() - 1) <= '9') {
					currentKanaStr = currentKanaStr.substring(0,
							currentKanaStr.length() - 1);
				}
				try {
					if (br.ready()) {
	...
	public void doExample() throws Exception {
		try {
			while ("\t[*]".equals(readbuffer.substring(0, 4))) {
				currentExampleStr = readbuffer.substring(4,
						readbuffer.indexOf('／'));
				currentExampleMeaningStr = readbuffer
						.substring(readbuffer.indexOf('／') + 1,
								readbuffer.indexOf("[/*]"));
				if (currentExampleMeaningStr.indexOf("[/trn]") > 0) {
					currentExampleMeaningStr = currentExampleMeaningStr
							.substring(0,
									currentExampleMeaningStr.indexOf("[/trn]"));
				}
			...
			if ("\t『語法』".equals(readbuffer.substring(0, "\t『語法』".length()))
					|| "\t『比較』".equals(readbuffer.substring(0,
							"\t『比較』".length()))
					|| "\t★".equals(readbuffer.substring(0, "\t★".length()))
					|| "\t『参考』".equals(readbuffer.substring(0,
							"\t『参考』".length()))
					|| "\t▼".equals(readbuffer.substring(0, "\t▼".length()))) {
				String remarkStr = readbuffer;
				readbuffer = br.readLine();
				while (readbuffer.charAt(0) == '\t'
						&& readbuffer.charAt(1) != '（'
						&& !"[*]"
								.equals(readbuffer.substring(1, "[*]".length()))) {
					remarkStr += readbuffer;
					readbuffer = br.readLine();
				}
				remarkStr = remarkStr.substring(1);
				if (remarkStr.indexOf("[/trn]") > 0) {
					remarkStr = remarkStr.substring(0,
							remarkStr.indexOf("[/trn]"));
				}
				if (remarkStr.indexOf("[/*]") > 0) {
					remarkStr = remarkStr.substring(0,
							remarkStr.indexOf("[/*]"));
				}
				writebuffer = "insert into JPWORDREMARK values('" + remarkrow
						+ "','" + wordrow + "','" + remarkStr + "');";
				System.out.println(writebuffer);
				bw.write(writebuffer);
				bw.newLine();
	...
	public static void main(String[] args) {
		...
		String inputFilePath = projectPath + "/dict/kana_edited.txt";
		String outputFilePath = projectPath + "/dict/output.sql";
